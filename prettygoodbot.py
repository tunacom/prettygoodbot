...
class PrettyGoodClient(discord.Client):
  ...
  async def on_ready(self):
    ...
    async for guild in guilds:
      # TODO(tuna): discord.py contains a type check that breaks if the server
      # has no verification level set. Explicitly set one to work around it.
      ...
      await guild.edit(
          name=name, icon=icon_data, verification_level=verification_level)
